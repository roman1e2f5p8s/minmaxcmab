...
class Alg:
    def __init__(self, feature_dim, n_arms):
        ...
        for arm in range(n_arms):
            self.C[arm] = 0.5 * np.identity(n=feature_dim, dtype=np.float64) # TODO: C,R,S the same/arm?
            ...
            self.P_inv[arm] = np.linalg.inv(self.S[arm]) # assuming H[arm] is zeros
    ...
    def choose_arm(self, features):
        for arm in range(self.n_arms):
            V = np.linalg.inv(self.S[arm]) + self.C[arm] @ self.P_inv[arm] @ self.C[arm].T # TODO: inv(S)
            # is const
            self.P_inv[arm] = V - V @ self.H[arm].T @ \
                    np.linalg.inv(np.linalg.inv(self.R[arm]) + self.H[arm] @ V @ self.H[arm].T) @ \
                    self.H[arm] @ V  # TODO: how is R updated? store inv(R) as it is const 
            self.Theta[arm] = self.P_inv[arm].dot(np.linalg.inv(V)).\
                    dot(self.C[arm]).dot(self.Theta[arm]) + \
                    self.P_inv[arm].dot(self.H[arm].T).dot(self.R[arm]).dot(self.Y[arm])
            self.p[arm] = np.dot(features[arm], self.Theta[arm]) + \
                    2.5 * np.sqrt(np.dot(self.P_inv[arm].dot(features[arm]), features[arm]))
