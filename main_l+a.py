...
def true_theta(n_trials, n_arms, feature_dim, stationary=True):
    ...
    for t in range(n_trials):
        if stationary:
            ...
            theta[t] = np.random.normal(size=(n_arms, feature_dim), scale=scale)
        # theta[t, BEST_ARMS] = theta[t, BEST_ARMS] + 1

    return theta


def reward_func(theta, context, noise):
...
TRUE_THETA = true_theta(N_TRIALS, N_ARMS, FEATURE_DIM, STATIONARY)
...
EXPECTED_REWARDS = np.array([
        np.max([reward_func(TRUE_THETA[t, arm], DATA[t, arm], NOISE[t]) for arm in range(N_ARMS)]) \
...
for t in range(T):
    ...
    if LINUCB:
        ...
        reward = reward_func(TRUE_THETA[t, chosen_arm], features[chosen_arm], NOISE[t])
